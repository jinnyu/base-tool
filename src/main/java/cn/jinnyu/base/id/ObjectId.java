...
/**
 * Copy from <a href="https://github.com/mongodb/mongo-java-driver/blob/master/bson/src/main/org/bson/types/ObjectId.java">MongoDB java driver</a>
 *
...
public final class ObjectId implements Comparable<ObjectId>, Serializable {
    ...
    public ObjectId(final byte[] bytes) {
        this(ByteBuffer.wrap(isTrueArgument(bytes, notNull("bytes", bytes).length == 12)));
    }
    ...
    public ObjectId(final ByteBuffer buffer) {
        ...
        isTrueArgument(buffer.remaining() >= OBJECT_ID_LENGTH);
    ...
    private static void isTrueArgument(final boolean condition) {
        if (!condition) {
            throw new IllegalArgumentException("state should be: buffer.remaining() >=12");
        }
    }

    private static <T> T isTrueArgument(final T value, final boolean condition) {
        if (!condition) {
